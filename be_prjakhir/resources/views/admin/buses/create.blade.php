...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    <div class="flex items-center gap-4 mb-6">
        ...
        <h1 class="text-xl font-bold text-gray-900">🚌 Tambah Bus</h1>
    </div>
    ...
    <form method="POST" action="{{ route('admin.buses.store') }}" class="space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="bus_name" value="{{ old('bus_name') }}" 
                       ...
                       placeholder="PO R.Tugal" required>
                @error('bus_name')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="date" name="end_date" value="{{ old('end_date') }}" 
                       ...
                       required>
                <p class="text-xs text-gray-400 mt-1">Jadwal akan digenerate otomatis untuk setiap tanggal di range ini</p>
                @error('end_date')
                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
                ...
                @foreach($defaultTimes as $time)
                    <label class="flex items-center gap-2 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition">
                        <input type="checkbox" name="departure_times[]" value="{{ $time }}"
                               {{ in_array($time, old('departure_times', [])) ? 'checked' : '' }}
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="seat" value="{{ old('seat', 40) }}" 
                       ...
                       min="1" required>
                <p class="text-xs text-gray-400 mt-1">Kursi akan digenerate otomatis dengan layout 2-2</p>
                @error('seat')
                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" name="duration_minutes" value="{{ old('duration_minutes', 120) }}" 
