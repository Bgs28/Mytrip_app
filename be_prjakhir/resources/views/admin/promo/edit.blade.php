...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    <div class="flex items-center gap-4 mb-6">
        ...
        <h1 class="text-xl font-bold text-gray-900">✏️ Edit Promo</h1>
    </div>

    <form method="POST" action="{{ route('admin.promo.update', $promo->id) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea name="description" rows="3" 
                      ...
                      placeholder="Deskripsi promo...">{{ old('description', $promo->description) }}</textarea>
            @error('description')
                <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
                ...
                <input type="number" step="0.01" name="discount_value" value="{{ old('discount_value', $promo->discount_value) }}" 
            ...
            <div id="max_discount_wrapper">
                ...
                <input type="number" step="0.01" name="max_discount" value="{{ old('max_discount', $promo->max_discount) }}" 
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
                ...
                <input type="number" step="0.01" name="min_purchase" value="{{ old('min_purchase', $promo->min_purchase) }}" 
            ...
            <div>
                ...
                <select name="target_type" 
                        ...
                        required>
                    <option value="all" {{ old('target_type', $promo->target_type) == 'all' ? 'selected' : '' }}>Semua</option>
                    <option value="bus" {{ old('target_type', $promo->target_type) == 'bus' ? 'selected' : '' }}>Bus</option>
                    <option value="train" {{ old('target_type', $promo->target_type) == 'train' ? 'selected' : '' }}>Kereta Api</option>
                    <option value="hotel" {{ old('target_type', $promo->target_type) == 'hotel' ? 'selected' : '' }}>Hotel</option>
                </select>
                @error('target_type')
                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                @enderror
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="datetime-local" name="start_date" value="{{ old('start_date', date('Y-m-d\TH:i', strtotime($promo->start_date))) }}" 
            ...
            <div>
                ...
                <input type="datetime-local" name="end_date" value="{{ old('end_date', date('Y-m-d\TH:i', strtotime($promo->end_date))) }}" 
