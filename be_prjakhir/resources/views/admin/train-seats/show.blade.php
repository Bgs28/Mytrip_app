...
@section('title', 'Detail Kursi Kereta - ' . $seat->seat_code)

@section('content')
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    <div class="flex items-center gap-4 mb-6">
        <a href="{{ route('admin.train-seats.index', ['train_id' => $seat->train_id]) }}" class="text-blue-600 hover:text-blue-800">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-gray-50 rounded-lg p-6 border border-gray-200">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-sm text-gray-500">Kereta</p>
                        <p class="text-lg font-semibold">{{ $seat->train->train_name ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-500">Rute</p>
                        <p class="text-lg">{{ $seat->train->from ?? '-' }} → {{ $seat->train->destination ?? '-' }}</p>
                    </div>
                </div>
            </div>
                    ...
                    @foreach($layout as $rowIndex => $row)
                        <div class="flex gap-2">
                            @foreach($row as $colIndex => $s)
                                @if($s && $s['id'] == $seat->id)
                                    <div class="w-14 h-14 flex items-center justify-center text-xs font-bold rounded-lg border-2 border-blue-500 bg-blue-100 text-blue-700 shadow-lg">
                                        {{ $s['seat_code'] }}
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-gray-50 rounded-lg border border-gray-200 p-6">
                ...
                <div class="space-y-3">
                    <a href="{{ route('admin.train-seats.edit', $seat->id) }}" 
                       class="block w-full bg-amber-600 hover:bg-amber-700 text-white text-center font-medium py-2 rounded-lg transition">
                        ✏️ Edit Kursi
                    </a>
                    <form action="{{ route('admin.train-seats.toggleAvailability', $seat->id) }}" method="POST">
                        @csrf
                        <button type="submit" 
                    ...
                    <form action="{{ route('admin.train-seats.destroy', $seat->id) }}" method="POST" 
                          onsubmit="return confirm('Apakah Anda yakin ingin menghapus kursi ini?')">
                        @csrf
                        @method('DELETE')
