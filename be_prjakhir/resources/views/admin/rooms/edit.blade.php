...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    <div class="flex items-center gap-4 mb-6">
        ...
        <h1 class="text-xl font-bold text-gray-900">✏️ Edit Kamar - {{ $room->room_number }}</h1>
    </div>
    ...
    <form method="POST" action="{{ route('admin.rooms.update', $room->id) }}" class="space-y-6" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="bed_type" 
                    ...
                    @foreach($bedTypes as $bed)
                        <option value="{{ $bed }}" {{ old('bed_type', $room->bed_type) == $bed ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="number" name="size" value="{{ old('size', $room->size) }}" 
        ...
        <div>
            ...
            <textarea name="description" rows="3" 
                      ...
                      placeholder="Deskripsi fasilitas kamar...">{{ old('description', $room->description) }}</textarea>
            @error('description')
                <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        @if($room->thumbnail)
            <div class="mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-sm text-gray-500">Untuk mengganti, upload thumbnail baru di bawah.</p>
                    </div>
                </div>
            </div>
        @endif
    ...
    @enderror
        ...
        @if($room->images && count($room->images) > 0)
            <div class="mb-4">
                ...
                <div class="flex flex-wrap gap-2" id="currentImages">
                    @foreach($room->images as $image)
                        <div class="relative group border rounded-lg p-1" data-image="{{ $image }}">
        ...
        <div>
            ...
            <div id="imagesPreview" class="mt-2 flex flex-wrap gap-2">
                <p class="text-sm text-gray-400">Belum ada gambar baru yang dipilih</p>
            </div>
            @error('images.*')
                <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
            @enderror
        ...
        <div class="flex gap-3 pt-4 border-t border-gray-200">
            ...
            <a href="{{ route('admin.rooms.index', ['hotel_id' => $room->hotel_id]) }}" 
...
<script>
    ...
    function markImageForDeletion(btn, image) {
    if (!confirm('Hapus gambar ini?')) return;
        
        // Toggle deletion status
        const parent = btn.parentElement;
        const isDeleted = parent.style.opacity === '0.3';
        
        if (isDeleted) {
            // Unmark for deletion
            deletedImages = deletedImages.filter(i => i !== image);
        ...
        // Update hidden input dengan array
    ...
    document.getElementById('images')?.addEventListener('change', function(e) {
        ...
        if (files.length === 0) {
            preview.innerHTML = '<p class="text-sm text-gray-400">Belum ada gambar baru yang dipilih</p>';
            return;
        }
    ...
    function removePreviewImage(btn) {
        ...
        if (previewContainer.children.length === 0) {
            previewContainer.innerHTML = '<p class="text-sm text-gray-400">Belum ada gambar baru yang dipilih</p>';
        }
        // Reset file input if no previews left
        if (previewContainer.children.length === 0 || 
            (previewContainer.children.length === 1 && previewContainer.children[0].tagName === 'P')) {
