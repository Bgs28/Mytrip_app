...
@section('title', 'Kelola Kursi Bus')
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-xl font-bold text-gray-900">🪑 Kelola Kursi Bus</h1>
            <p class="text-sm text-gray-500 mt-1">
                @if($bus)
                    Kursi untuk bus: <span class="font-semibold text-gray-700">{{ $bus->bus_name }}</span>
                    <span class="text-xs text-gray-400 ml-2">(Layout 2-2 • Otomatis tergenerate)</span>
                @else
                    Kelola semua kursi bus | Pilih bus untuk melihat layout
                @endif
            </p>
        </div>
        @if($bus)
            <form action="{{ route('admin.bus-seats.regenerate') }}" method="POST" 
    ...
    <!-- Filter -->
    <div class="mb-6">
        <form method="GET" action="{{ route('admin.bus-seats.index') }}" class="flex flex-wrap gap-3">
            @if($bus)
                <input type="hidden" name="bus_id" value="{{ $bus->id }}">
            @endif
            <select name="bus_id" class="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">
            ...
            <select name="seat_type" class="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">
    ...
    @if($bus)
        <div class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
            <div class="flex items-center justify-between mb-3">
                ...
                <span class="text-xs text-gray-400">Total: {{ $bus->seats->count() }} kursi</span>
            </div>
            <div class="flex flex-wrap gap-2">
                ...
                @if(count($layout) > 0)
                    ...
                    @endforeach
                @else
                    <p class="text-sm text-gray-400">Belum ada kursi. <a href="{{ route('admin.buses.edit', $bus->id) }}" class="text-blue-600 hover:underline">Edit bus</a> untuk mengatur jumlah kursi.</p>
                @endif
            </div>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-6 py-4 text-gray-700">
                        <a href="{{ route('admin.bus-seats.index', ['bus_id' => $seat->bus_id]) }}" class="text-blue-600 hover:underline">
                            {{ $seat->bus->bus_name ?? '-' }}
                        </a>
                    </td>
                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="flex items-center justify-center gap-1">
                            <a href="{{ route('admin.bus-seats.show', $seat->id) }}" 
                            ...
                            <form action="{{ route('admin.bus-seats.toggleAvailability', $seat->id) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" 
                ...
                <tr>
                    <td colspan="8" class="px-6 py-10 text-center text-gray-400">
                        @if($bus)
                            Belum ada kursi untuk bus ini. 
                            <a href="{{ route('admin.buses.edit', $bus->id) }}" class="text-blue-600 hover:underline">Edit bus</a> 
                            untuk mengatur jumlah kursi, atau klik 
                            <form action="{{ route('admin.bus-seats.regenerate') }}" method="POST" class="inline">
                                @csrf
                                <input type="hidden" name="bus_id" value="{{ $bus->id }}">
                                <button type="submit" class="text-blue-600 hover:underline font-medium">Regenerate</button>
                            </form>
                        @else
                            Belum ada data kursi bus. Pilih bus dari filter di atas untuk melihat kursinya.
                        @endif
                    </td>
                </tr>
                @endforelse
